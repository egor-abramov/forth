...
- `if ... else ... then` - выполнить инструкции после `if` если, на стеке единица, иначе выполнить инструкции после
...
как ASCII-символ, в зависимости от инструкции.
Поддерживаются числовые литералы (`var`), и константные строки (`string`)

### Контракты
Каждая процедура обязана декларировать стековый контракт. Контракты обеспечивают безопасность работы со стеком, предотвращают утечки стека, stack underflow и т.д.
...
### Анализ стека в ветвлениях и циклах
Для надежности языка компилятор использует статический анализатор стека. Анализатор производит проверку на утечки стека и ошибки опустошения стека. Особого внимания требует проверка ветвлений и циклов.

**Ветвления**

Цель анализатора - гарантировать, что вне зависимости от пути выполнения на стеке окажется одинаковой количество элементов.
То есть ветки `if` и `else` должны оказывать на стек одинаковый эффект. При невыполнении этого условия программа не будет скомпилирована. Такая проверка необходима так как, в случае если ветки дают разный эффект, становится невозможным дальнейший анализ стека и проверка контрактов.

**Циклы**

После выполнения тела цикла на стеке должно остаться на один элемент больше, чем было до. 
Этот элемент в конце каждой итерации снимается с вершины и используется `endloop` как флаг продолжения исполнения. 
То есть цикл не должен оставлять после себя элементы на стеке. 
Такой подход позволяет избежать утечек памяти. 
В случае, если цикл оставляет после себя элементы на стеке, программа не будет скомпилирована
...
- `read_array` - чтение массива, по адресу со стека
...
### Плагин
Для удобства написания кода в ide от JetBrains реализован [плагин](https://github.com/egor-abramov/forthLSP). Плагин анализирует глубину стека, соблюдение контрактов, корректность ветвлений и циклов, а также подсвечивает иснтаксис.
...
Строки хранятся в формате *Pascal-строк*. Каждый элемент строки занимает машинное слово *целиком* (4 байта). Символы
...
* Add Immediate
    * `addi <rd>, <rs>, <k>`
    * 2 такта
    * Прибавляет к значению в регистре `rs` младшие 12 бит `k`, расширяя знак `k`. Результат сохраняется в регистр `rd`
...
Интерфейс командной строки: `go run cmd/machine/main.go [-scalar] <source_path> <input_path>`. Флаг `-scalar` является опциональным, при его использовании исполнение будет происходить в параллельном режиме, по исполнение происходит в суперскалярном режиме.
...
На каждом такте из Data Path приходят по две инструкции (данные из регистров `IR1`, `IR2`) и флаги состояния.

**Компоненты Control Unit**

- `mPC 1/2` - счетчики микрокоманд
- `MUX mPC 1/2` - мультиплексор, выбирающий следующее значение `mPC`. Новым значением может быть инкремент, 0, результат
...
- `Sync Logic` - принимает `mPC1` и `mPC2`. Если выполнение одной из инструкций завершилось (`mPC` установлен в 0), а
  другой еще нет, то на соответствующий мультиплексор подается сигнал `force_wait`, устанавливающий значение `mPC` на
...
Инструкции могут исполняться за разное количество тактов. В таком случае, более быстрая инструкция будет находиться в
...
- `sort` - сортировка массива из потока ввода
